...
from others.data_user import get_data_state, get_data_for_admin, paste_data_db
...
async def application(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer('Оставьте ваш номер телефона :)')



@rout.callback_query(F.data.startswith('add_service'), States.state_add_service)
async def handler_add_servise(call: CallbackQuery, state: FSMContext, requests: Requests):
    add = []
...
@rout.callback_query(States.hand_enter_number)
async def past_phone_number(call: CallbackQuery, state: FSMContext):     
    ...
...
async def check_phone(message: Message, state: FSMContext, bot: Bot, requests: Requests):
    ...
    # await state.update_data(state_get_phone= message.contact.phone_number)
    await state.update_data(hand_enter_number= message.text)
    await message.answer('Спасибо за ваш заказ! Ваши данные переданы менеджеру!')
    data = await state.get_data()
    ...
    await paste_data_db(state, requests)
